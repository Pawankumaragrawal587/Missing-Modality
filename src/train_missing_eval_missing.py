...
def main(args):
    ...
    sound_mean = sound_mean.astype(np.float32)
    ...
    for iterate in range(args.iterations):
        ...
        # if iterate == 0:
        #     test_image_acc_i = eval_image_test(criterion_meta_train, meta_test_loader, image_sound_extractor, encoder, sound_mean, device, writer)
            
        #     mtr_image_acc_i = eval_image_train(criterion_meta_train, meta_train_loader, image_sound_extractor, encoder, sound_mean, device, writer)
        #     mval_image_acc_i = eval_image_test(criterion_meta_train, meta_val_loader, image_sound_extractor, encoder, sound_mean, device, writer)
        #     torch.set_grad_enabled(True)
        #     print('Initial Model  RMSE: mtr image RMSE:{:.4f}, mval image RMSE:{:.4f}, image-only test RMSE:{:.4f}' .format(mtr_image_acc_i, mval_image_acc_i, test_image_acc_i))
        ...
        if (iterate) % 2000 == 0 or iterate == args.iterations - 1:
            ...
            test_image_acc = 0.
            for i in range(5):
                test_image_acc += eval_image_test(criterion_meta_train, meta_test_loader, image_sound_extractor, encoder, sound_mean, device, writer)
            test_image_acc = test_image_acc / 5

            mtr_image_acc = eval_image_train(criterion_meta_train, meta_train_loader, image_sound_extractor, encoder, sound_mean, device, writer)
            mval_image_acc = eval_image_test(criterion_meta_train, meta_val_loader, image_sound_extractor, encoder, sound_mean, device, writer)
            torch.set_grad_enabled(True)
            print('Iteration:[{}/{}], mtr image RMSE:{:.6f}, mval image RMSE:{:.6f}, image-only test RMSE:{:.6f}' .format(iterate, args.iterations, mtr_image_acc, mval_image_acc, test_image_acc))
...
def meta_training(args, meta_train_batch, meta_val_batch, sound_mean, image_sound_extractor, encoder, criterion_meta_train, criterion_meta_val, optimizer_image_sound, optimizer_encoder, device, writer, iterate, total_iterate):
    ...
    meta_train_user_embedding = meta_train_batch[1].to(device)
    meta_train_label = meta_train_batch[2].to(device)
    ...
    meta_val_user_embedding = meta_val_batch[2].to(device)
    meta_val_label = meta_val_batch[3].to(device)
    ...
    for idx in range(args.inner_loop):
        ...
        pred_meta_train_noised,_,_,_ = image_sound_extractor.functional(params_star, True)(meta_train_label, meta_train_user_embedding, meta_train_image, sound=None, encoder=encoder,  sound_mean=sound_mean, noise_layer=['fc0','fc1','fc2'], meta_train=True, mode='one')
    ...
    pred_meta_val_noised, f_meta_val_noised1,f_meta_val_noised2, sound_mean_val_noised = image_sound_extractor.functional(params_star, True)(meta_val_label, meta_val_user_embedding, meta_val_image, sound=None, encoder=encoder, sound_mean=sound_mean, noise_layer=['fc0','fc1','fc2'], meta_train=False, mode='one')

    pred_meta_val_clean, f_meta_val_clean1,f_meta_val_clean2, sound_mean_val_clean = image_sound_extractor.functional(params_star, True)(meta_val_label, meta_val_user_embedding, meta_val_image, sound=meta_val_sound, mode='two')
...
def eval_image_test(criterion_meta_train, test_loader, image_sound_extractor, encoder,sound_mean, device, writer):
    ...
    for i, batch in enumerate(test_loader):
        ...
        user_embeddings = batch[2].to(device)
        labels = batch[3].to(device)
        labels = torch.reshape(labels, (labels.shape[0], 1))

        outputs,_,_,_ = image_sound_extractor(labels, user_embeddings, images, sound=sounds, encoder=encoder,  sound_mean=sound_mean, noise_layer=['fc0','fc1','fc2'], meta_train=False, mode='one')
...
def eval_image_train(criterion_meta_train, test_loader, image_sound_extractor, encoder,sound_mean, device, writer):
    # Switch to evaluate mode
    torch.set_grad_enabled(False)
    image_sound_extractor.eval()
    loss = 0
    ...
    for i, batch in enumerate(test_loader):
        ...
        images = batch[0].to(device)
        user_embeddings = batch[1].to(device)
        labels = batch[2].to(device)
        labels = torch.reshape(labels, (labels.shape[0], 1))

        outputs,_,_,_ = image_sound_extractor(labels, user_embeddings, images, sound=None, encoder=encoder,  sound_mean=sound_mean, noise_layer=['fc0','fc1','fc2'], meta_train=False, mode='one')
